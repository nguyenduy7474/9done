	...
	.then(res => {
		if(res == "Not Found User"){
			...
			for(var i=0; i<allsongs.length; i++){
				...
				listsong +=`<div class="jumplentren" onclick="chooseSong('${allsongs[i].handledname}','${userinfo.id}', '${allsongs[i]._id}')" style="color: white; font-size: 110%; text-decoration: none"><video playsinline style="width: 100%; height: 52.25%" src="/users/${userinfo.id}/songhandled/${allsongs[i].handledname}#t=0.1"></video></br>${allsongs[i].songname}<p style="color: #989797; font-size: 90%;">${allsongs[i].view} lượt nghe • ${date}</p></div>`
			}
